...
class AudioController {
    ...
    playHit() { this.playTone(150, 'sawtooth', 0.2, 0.1); }
    playExplosion() { this.playTone(50, 'sawtooth', 0.5, 0.3); }
    playLevelUp() {
...
let stats = JSON.parse(localStorage.getItem('gameStats')) || { totalKills: 0, bossesDefeated: 0, highestLevelReached: 1 };
...
class Player {
    constructor(x, y) {
        this.x = x; this.y = y; this.radius = 15; this.color = '#66fcf1'; this.speed = 200;
...
class Enemy {
    constructor(x, y, type = 1) {
        ...
        if (type === 2) { this.radius = 18; this.color = '#ffcc00'; this.speed *= 1.2; this.maxHealth *= 3; this.xpValue = 30; } 
        else if (type === 3) { this.radius = 50; this.color = '#ff4400'; this.speed = 40 * currentLevel.enemySpeedMult; this.maxHealth = 1500 * currentLevel.enemyHealthMult; this.damage = 30 * scale; this.xpValue = 500; }
    ...
    draw(ctx) {
        ctx.shadowBlur = this.type === 3 ? 30 : 10; ctx.shadowColor = this.color; ctx.beginPath();
        if (this.type === 3) {
            for (let i = 0; i < 8; i++) {
...
class Projectile {
    constructor(x, y, angle, speed, damage, type) {
        ...
        this.color = colorMap[type]; this.hitSet = new Set();
    }
    ...
    draw(ctx) {
        ...
        } else { ctx.arc(this.x, this.y, this.radius, 0, Math.PI * 2); ctx.fillStyle = this.color; ctx.fill(); }
        ctx.shadowBlur = 0;
    }
}
...
class Particle {
    constructor(x, y, color) {
        ...
        this.life = 0.5 + Math.random() * 0.5; this.maxLife = this.life; this.radius = Math.random() * 3 + 1;
    }
...
function gameLoop(timestamp) {
    ...
    if (screenShakeAmount > 0) {
        ...
        screenShakeAmount -= dt * 30; // decay
        if (screenShakeAmount < 0) screenShakeAmount = 0;
    }

    ctx.fillStyle = 'rgba(11, 12, 16, 0.4)'; ctx.fillRect(-50, -50, canvas.width+100, canvas.height+100);
...
function renderAchievements() {
    ...
    achievementsConfig.forEach(ach => {
        ...
        item.innerHTML = `<div><h3>${ach.name}</h3><p>${ach.desc}</p></div><div class="achievement-reward">${isDone ? 'UNLOCKED' : 'LOCKED'}<br><span style="font-size:0.8rem; font-weight:normal; color:#fff;">Reward: ${ach.reward}</span></div>`;
...
uiElements.toggleSoundBtn.addEventListener('click', () => {
    audio.enabled = !audio.enabled;
    uiElements.toggleSoundBtn.innerText = audio.enabled ? "🔊 ON" : "🔇 OFF";
